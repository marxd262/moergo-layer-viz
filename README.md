...
- **Windows: Raw HID requires USB.** Over Bluetooth LE, the Windows HoGP (HID-over-GATT) kernel driver claims the standard BLE HID service (UUID `0x1812`) exclusively and refuses both the standard HID class API (HidSharp / `hidapi`) and direct WinRT GATT access (`AccessDenied` even with `GattSharingMode.SharedReadAndWrite`). Since `zzeneg/zmk-raw-hid` registers its raw-HID reports inside `0x1812`, the FF60/61 reports are unreachable from any user-mode app on Windows when the board is connected over BLE. macOS and Linux are unaffected — IOHIDManager and `/dev/hidraw*` both expose vendor collections regardless of transport. Workaround: connect via USB (USB Raw HID works fully on Windows), or fall back to the signal-macro path which works over both transports. The proper fix has to come from firmware — exposing the raw-HID reports under a custom GATT service UUID in parallel with `0x1812` would side-step HoGP entirely.
